...
import { fileToBase64, validateImageFile, formatFileSize } from '../../utils/fileUtils';
...
export function LogoUpload({ 
  ...
  const handleTypeChange = useCallback((type: 'url' | 'file') => {
        ...
        <div className="relative">
          ...
          <label
            ...
            className={`flex flex-col items-center justify-center w-full h-32 px-4 py-6 border-2 border-dashed rounded-lg cursor-pointer transition-colors
                     ${error ? 'border-red-300 hover:border-red-500 hover:bg-red-50' :
                              'border-gray-300 hover:border-primary-500 hover:bg-primary-50'}
                     ${isLoading ? 'opacity-50 cursor-wait' : ''}`}
          >
            <Image className={`w-8 h-8 mb-2 ${error ? 'text-red-400' : 'text-gray-400'}`} />
